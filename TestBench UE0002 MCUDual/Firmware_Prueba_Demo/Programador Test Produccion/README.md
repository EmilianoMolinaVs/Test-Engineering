# Programador Test Producción - DualMCU ESP32 + RP2040

Este directorio contiene las herramientas y los binarios finales usados por el equipo de fabricación para programar las placas DualMCU (ESP32 + RP2040) de Uelectronics.

**Producto**: [Unit DualMCU ESP32 RP2040 Tarjeta de Desarrollo](https://uelectronics.com/producto/unit-dualmcu-esp32-rp2040-tarjeta-de-desarrollo/)

**Propósito**: flashear lotes de dispositivos de forma rápida y reproducible.

**Archivos importantes en esta carpeta**
- `blink_DualMCU_ESP32.ino.merged.bin` : binario final para ESP32 en la placa DualMCU.
- `blink_DualMCU_RP2040.ino.uf2` : firmware final para RP2040 en la placa DualMCU.
...
- Conectar las placas DualMCU por USB. Para el ESP32, conectar normalmente. Para el RP2040, conectar cada placa en modo BOOTSEL (aparecerán como unidades removibles tipo RPI-RP2).
...
- 3 Grabar firmware (usa `blink_DualMCU_ESP32.ino.merged.bin` por defecto).
...
- Copia el archivo UF2 (`blink_DualMCU_RP2040.ino.uf2`) a cada unidad detectada en paralelo. Las placas se reinician automáticamente después de copiar.
...
```bash
cd "C:/GitHub Repositorios/Test-Engineering/TestBench UE0002 MCUDual/Firmware_Prueba_Demo/Programador Test Produccion"
./flash_multi_esp32.sh
./flash_multi_rp2040.sh
```
